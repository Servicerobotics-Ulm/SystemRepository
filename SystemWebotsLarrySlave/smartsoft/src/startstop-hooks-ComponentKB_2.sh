#!/bin/bash
#
# Start-Stop-Hooks for component ComponentKB_2
#
...
pre-start)
	...
	echo $(date +%T)
        # Update the knowledge base ini param to connect larry kb event client to master event server
	sed -i -z 's/wiringName kbChainedEntriesEventClient\nserverName unknown\nserviceName unknown/wiringName kbChainedEntriesEventClient\nserverName ComponentKB_3\nserviceName kbEventServer/g' ComponentKB_2.ini
	echo $(date +%T)
;;
